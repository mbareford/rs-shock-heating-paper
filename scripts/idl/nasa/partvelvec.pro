...
;
;       CENTER - Set this keyword to center vector over particle position
;
...
PRO partvelvec,velx,vely,posx,posy,x,y, OVER = over, VECCOLORS=vecColors, $
               FRACTION=fraction,LENGTH=length,COLOR=color,CENTER=center,WINDOW=window, $
	       _EXTRA=extra
...
FOR i=0l,nvecs-1l DO BEGIN  ; Loop over particles.
    ...
    IF (vel[i] EQ 0.0d0) THEN BEGIN
      ;cgPlots,[px[i],x1[i],x1[i], x1[i],x1[i]], $
      ;   [py[i],y1[i],y1[i], y1[i],y1[i]],COLOR=veccolors[i],$
	  ;ADDCMD = window
    ENDIF ELSE BEGIN
      IF keyword_set(center) THEN BEGIN
        px[i] = px[i] + 0.5d0*(px[i]-x1[i])
        py[i] = py[i] + 0.5d0*(py[i]-y1[i])
        x1[i] = x1[i] + 0.5d0*(px[i]-x1[i])
        y1[i] = y1[i] + 0.5d0*(py[i]-y1[i])
      ENDIF 
      
      cgPlots,[px[i],x1[i],x1[i]-(vx[i]*rcos+vy[i]*rsin)/vel[i], x1[i],x1[i]-(vx[i]*rcos-vy[i]*rsin)/vel[i]], $
              [py[i],y1[i],y1[i]-(vy[i]*rcos-vx[i]*rsin)/vel[i], y1[i],y1[i]-(vy[i]*rcos+vx[i]*rsin)/vel[i]],COLOR=veccolors[i],$
