MODULE lagran
  ...
  REAL(num), DIMENSION(:, :, :), ALLOCATABLE :: bx_cc, by_cc, bz_cc 
  REAL(num), DIMENSION(:, :, :), ALLOCATABLE :: bx1_cc, by1_cc, bz1_cc 
  REAL(num), DIMENSION(:, :, :), ALLOCATABLE :: qxy, qxz, qyz, pressure
  REAL(num), DIMENSION(:, :, :), ALLOCATABLE :: qxx, qyy, qzz
...
CONTAINS
  ...
  SUBROUTINE lagrangian_step
    ...
    ALLOCATE (bx1(0:nx+1, 0:ny+1, 0:nz+1), by1(0:nx+1, 0:ny+1, 0:nz+1), bz1(0:nx+1, 0:ny+1, 0:nz+1), &
        bx_cc(0:nx+1, 0:ny+1, 0:nz+1), by_cc(0:nx+1, 0:ny+1, 0:nz+1), bz_cc(0:nx+1, 0:ny+1, 0:nz+1), &
        bx1_cc(0:nx+1, 0:ny+1, 0:nz+1), by1_cc(0:nx+1, 0:ny+1, 0:nz+1), bz1_cc(0:nx+1, 0:ny+1, 0:nz+1), &
    ...
    ! measure the magnetic dissipation due to averaging
    DO iz = 0, nz+1
      izm = iz - 1
      DO iy = 0, ny+1
        iym = iy - 1
        DO ix = 0, nx+1
          ixm = ix - 1
          bx_cc(ix, iy, iz) = (bx(ix, iy, iz) + bx(ixm, iy, iz)) * 0.5_num
          by_cc(ix, iy, iz) = (by(ix, iy, iz) + by(ix, iym, iz)) * 0.5_num
          bz_cc(ix, iy, iz) = (bz(ix, iy, iz) + bz(ix, iy, izm)) * 0.5_num
        END DO
      END DO
    END DO    
    tdbe1 = 0.0_num
    DO iz = 1, nz
      ...
      DO iy = 1, ny
        ...
        DO ix = 1, nx   
          ixm = ix - 1
          w1 = (bx(ix,iy,iz)**2 + bx(ixm,iy,iz)**2)/2.0_num
          w2 = (by(ix,iy,iz)**2 + by(ix,iym,iz)**2)/2.0_num
          w3 = (bz(ix,iy,iz)**2 + bz(ix,iy,izm)**2)/2.0_num
          w4 = (w1 + w2 + w3)*(cv(ix,iy,iz)/2.0_num)
          w5 = (bx_cc(ix,iy,iz)**2 + by_cc(ix,iy,iz)**2 + bz_cc(ix,iy,iz)**2)*(cv(ix,iy,iz)/2.0_num)
          tdbe1 = tdbe1 + w5-w4
        END DO
      END DO
    END DO
    ...
    ! measure the magnetic dissipation due to resistivity
    DO iz = 0, nz+1
      izm = iz - 1
      DO iy = 0, ny+1
        iym = iy - 1
        DO ix = 0, nx+1
          ixm = ix - 1
          bx1_cc(ix, iy, iz) = (bx(ix, iy, iz) + bx(ixm, iy, iz)) * 0.5_num
          by1_cc(ix, iy, iz) = (by(ix, iy, iz) + by(ix, iym, iz)) * 0.5_num
          bz1_cc(ix, iy, iz) = (bz(ix, iy, iz) + bz(ix, iy, izm)) * 0.5_num
        END DO
      END DO
    END DO
    tdbe2 = 0.0_num
    DO iz = 1, nz
      DO iy = 1, ny
        DO ix = 1, nx             
          w4 = (bx_cc(ix,iy,iz)**2 + by_cc(ix,iy,iz)**2 + bz_cc(ix,iy,iz)**2)*(cv(ix,iy,iz)/2.0_num)
          w5 = (bx1_cc(ix,iy,iz)**2 + by1_cc(ix,iy,iz)**2 + bz1_cc(ix,iy,iz)**2)*(cv(ix,iy,iz)/2.0_num)
          tdbe2 = tdbe2 + w5-w4
        END DO
      END DO
    END DO

    IF (conduction .OR. radiation) THEN
      IF (conrad_half_dt) THEN
        dt = dt / 2.0_num
      END IF
      IF (conduction .OR. (radiation .AND. rad_implicit)) CALL conduct
      IF (radiation .AND. (.NOT. rad_implicit)) CALL radiate
      IF (conrad_half_dt) THEN
        dt = 2.0_num*dt
      END IF
    END IF
    ...
    DEALLOCATE (bx1, by1, bz1, bx_cc, by_cc, bz_cc, bx1_cc, by1_cc, bz1_cc, qxy, qxz, qyz, pressure, qxx, qyy, &
  ...
  SUBROUTINE predictor_corrector_step
    ...
    REAL(num) :: e1, rho_v, mass_v, cv_v
    REAL(num) :: fx, fy, fz, tfx, tfy, tfz
    REAL(num) :: f1, f2, f3 
    REAL(num) :: fx1, fx2, fx3, fy1, fy2, fy3, fz1, fz2, fz3
    REAL(num) :: vxb, vxbm, vyb, vybm, vzb, vzbm
    REAL(num) :: bxv, byv, bzv, jx, jy, jz
    REAL(num) :: cvx, cvxp, cvy, cvyp, cvz, cvzp
    REAL(num) :: dv, msc

    REAL(num) :: vx_xfc, vx_yfc, vx_zfc, vx_xfc_m, vx_yfc_m, vx_zfc_m
    REAL(num) :: vy_xfc, vy_yfc, vy_zfc, vy_xfc_m, vy_yfc_m, vy_zfc_m
    REAL(num) :: vz_xfc, vz_yfc, vz_zfc, vz_xfc_m, vz_yfc_m, vz_zfc_m

    REAL(num) :: dx_vx, dy_vx, dz_vx
    REAL(num) :: dx_vy, dy_vy, dz_vy
    REAL(num) :: dx_vz, dy_vz, dz_vz

    REAL(num) :: dxb_dt, dyb_dt, dzb_dt


    ! cell-centred field components with resistivity applied
    bx_cc = bx1
    by_cc = by1
    bz_cc = bz1
    ...
    ! measure the magnetic dissipation due to evolving induction equation over half the timestep
    tdbe3 = 0.0_num
    DO iz = 1, nz
      DO iy = 1, ny
        DO ix = 1, nx   
          w4 = (bx_cc(ix,iy,iz)**2 + by_cc(ix,iy,iz)**2 + bz_cc(ix,iy,iz)**2)*(cv(ix,iy,iz)/2.0_num)
          w5 = (bx1(ix,iy,iz)**2 + by1(ix,iy,iz)**2 + bz1(ix,iy,iz)**2)*(cv1(ix,iy,iz)/2.0_num)
          tdbe3 = tdbe3 + w5-w4
        END DO
      END DO
    END DO
    ...
    DO iz = 0, nz
      ...
      DO iy = 0, ny
        ...
        DO ix = 0, nx
          ...
          mass_v = rho(ix , iy , iz ) * cv(ix , iy , iz ) &
          ...
          rho_v = mass_v / cv_v
          msc = 0.5_num*rho_v*(cv_v/8.0_num)
          ...
          w1 = msc*(vx(ix,iy,iz)**2 + vy(ix,iy,iz)**2 + vz(ix,iy,iz)**2)         
                              
          ! find half step velocity needed for remap
          vx1(ix, iy, iz) = vx(ix, iy, iz) + dt2 * tfx / rho_v
          vy1(ix, iy, iz) = vy(ix, iy, iz) + dt2 * tfy / rho_v
          vz1(ix, iy, iz) = vz(ix, iy, iz) + dt2 * tfz / rho_v          

          w2 = msc*(vx1(ix,iy,iz)**2 + vy1(ix,iy,iz)**2 + vz1(ix,iy,iz)**2)

          ! velocity at the end of the Lagrangian step
          vx(ix, iy, iz) = vx(ix, iy, iz) + dt * tfx / rho_v
          vy(ix, iy, iz) = vy(ix, iy, iz) + dt * tfy / rho_v
          vz(ix, iy, iz) = vz(ix, iy, iz) + dt * tfz / rho_v
    ...
    DO iz = 1, nz
      ...
      DO iy = 1, ny
        ...
        DO ix = 1, nx
          ...
          vx_xfc = (vx1(ix,iy,iz) + vx1(ix,iym,iz) + vx1(ix,iy,izm) + vx1(ix,iym,izm))*0.25_num
          vy_xfc = (vy1(ix,iy,iz) + vy1(ix,iym,iz) + vy1(ix,iy,izm) + vy1(ix,iym,izm))*0.25_num
          vz_xfc = (vz1(ix,iy,iz) + vz1(ix,iym,iz) + vz1(ix,iy,izm) + vz1(ix,iym,izm))*0.25_num

          vx_xfc_m = (vx1(ixm,iy,iz) + vx1(ixm,iym,iz) + vx1(ixm,iy,izm) + vx1(ixm,iym,izm))*0.25_num
          vy_xfc_m = (vy1(ixm,iy,iz) + vy1(ixm,iym,iz) + vy1(ixm,iy,izm) + vy1(ixm,iym,izm))*0.25_num
          vz_xfc_m = (vz1(ixm,iy,iz) + vz1(ixm,iym,iz) + vz1(ixm,iy,izm) + vz1(ixm,iym,izm))*0.25_num

          vx_yfc = (vx1(ix,iy,iz) + vx1(ixm,iy,iz) + vx1(ix,iy,izm) + vx1(ixm,iy,izm))*0.25_num
          vy_yfc = (vy1(ix,iy,iz) + vy1(ixm,iy,iz) + vy1(ix,iy,izm) + vy1(ixm,iy,izm))*0.25_num
          vz_yfc = (vz1(ix,iy,iz) + vz1(ixm,iy,iz) + vz1(ix,iy,izm) + vz1(ixm,iy,izm))*0.25_num

          vx_yfc_m = (vx1(ix,iym,iz) + vx1(ixm,iym,iz) + vx1(ix,iym,izm) + vx1(ixm,iym,izm))*0.25_num
          vy_yfc_m = (vy1(ix,iym,iz) + vy1(ixm,iym,iz) + vy1(ix,iym,izm) + vy1(ixm,iym,izm))*0.25_num
          vz_yfc_m = (vz1(ix,iym,iz) + vz1(ixm,iym,iz) + vz1(ix,iym,izm) + vz1(ixm,iym,izm))*0.25_num

          vx_zfc = (vx1(ix,iy,iz) + vx1(ixm,iy,iz) + vx1(ix,iym,iz) + vx1(ixm,iym,iz))*0.25_num
          vy_zfc = (vy1(ix,iy,iz) + vy1(ixm,iy,iz) + vy1(ix,iym,iz) + vy1(ixm,iym,iz))*0.25_num
          vz_zfc = (vz1(ix,iy,iz) + vz1(ixm,iy,iz) + vz1(ix,iym,iz) + vz1(ixm,iym,iz))*0.25_num         

          vx_zfc_m = (vx1(ix,iy,izm) + vx1(ixm,iy,izm) + vx1(ix,iym,izm) + vx1(ixm,iym,izm))*0.25_num
          vy_zfc_m = (vy1(ix,iy,izm) + vy1(ixm,iy,izm) + vy1(ix,iym,izm) + vy1(ixm,iym,izm))*0.25_num
          vz_zfc_m = (vz1(ix,iy,izm) + vz1(ixm,iy,izm) + vz1(ix,iym,izm) + vz1(ixm,iym,izm))*0.25_num

          dv = ((vx_xfc - vx_xfc_m)/dxb(ix) + (vy_yfc - vy_yfc_m)/dyb(iy) + (vz_zfc - vz_zfc_m)/dzb(iz))*dt
          ...
          heating_dp = heating_dp - pressure(ix, iy, iz) * dv * cv(ix, iy, iz)                  
                
#ifdef Q_MONO
          heating_visc = heating_visc - p_visc(ix, iy, iz) * dv * cv(ix, iy, iz)       
#else      
          heating_visc = heating_visc + dt * visc_heat(ix, iy, iz) * cv(ix, iy, iz) 
          
          htvisc_xy = htvisc_xy + dt*visc_heat_xy(ix,iy,iz)*cv(ix,iy,iz)
          htvisc_xz = htvisc_xz + dt*visc_heat_xz(ix,iy,iz)*cv(ix,iy,iz)
          htvisc_yz = htvisc_yz + dt*visc_heat_yz(ix,iy,iz)*cv(ix,iy,iz)
          htvisc_xx = htvisc_xx + dt*visc_heat_xx(ix,iy,iz)*cv(ix,iy,iz)
          htvisc_yy = htvisc_yy + dt*visc_heat_yy(ix,iy,iz)*cv(ix,iy,iz)
          htvisc_zz = htvisc_zz + dt*visc_heat_zz(ix,iy,iz)*cv(ix,iy,iz)
#endif
                    

          ! measure magnetic dissipation due to evolution over full time step
          ! calculate cell expansions/compressions representative of time step
          dxb_dt = dxb(ix) + (vx_xfc - vx_xfc_m)*dt
          dyb_dt = dyb(iy) + (vy_yfc - vy_yfc_m)*dt
          dzb_dt = dzb(iz) + (vz_zfc - vz_zfc_m)*dt

          dx_vx = (vx_xfc - vx_xfc_m)/dxb_dt
          dx_vy = (vy_xfc - vy_xfc_m)/dxb_dt
          dx_vz = (vz_xfc - vz_xfc_m)/dxb_dt

          dy_vx = (vx_yfc - vx_yfc_m)/dyb_dt         
          dy_vy = (vy_yfc - vy_yfc_m)/dyb_dt
          dy_vz = (vz_yfc - vz_yfc_m)/dyb_dt

          dz_vx = (vx_zfc - vx_zfc_m)/dzb_dt
          dz_vy = (vy_zfc - vy_zfc_m)/dzb_dt
          dz_vz = (vz_zfc - vz_zfc_m)/dzb_dt         
         
          w1 = bx_cc(ix,iy,iz)*dx_vx + by_cc(ix,iy,iz)*dy_vx + bz_cc(ix,iy,iz)*dz_vx
          bx1_cc(ix,iy,iz) = (bx_cc(ix,iy,iz) + w1*dt)/(1.0_num + dv)

          w1 = bx_cc(ix,iy,iz)*dx_vy + by_cc(ix,iy,iz)*dy_vy + bz_cc(ix,iy,iz)*dz_vy
          by1_cc(ix,iy,iz) = (by_cc(ix,iy,iz) + w1*dt)/(1.0_num + dv)

          w1 = bx_cc(ix,iy,iz)*dx_vz + by_cc(ix,iy,iz)*dy_vz + bz_cc(ix,iy,iz)*dz_vz
          bz1_cc(ix,iy,iz) = (bz_cc(ix,iy,iz) + w1*dt)/(1.0_num + dv)
          
          w4 = (bx_cc(ix,iy,iz)**2 + by_cc(ix,iy,iz)**2 + bz_cc(ix,iy,iz)**2)*(cv(ix,iy,iz)/2.0_num)
          w5 = (bx1_cc(ix,iy,iz)**2 + by1_cc(ix,iy,iz)**2 + bz1_cc(ix,iy,iz)**2)*(cv1(ix,iy,iz)/2.0_num)
          tdbe4 = tdbe4 + w5-w4                                  
  ...
  SUBROUTINE viscosity_and_b_update
    ...
    REAL(num) :: tpc, tpc_si, tpc_ev, nc_si, nc_cgs, ld_si, cl, ti
...
!$		!*****************
          ...
          visc_heat_xy(ix,iy,iz) = qxy(ix,iy,iz)*dvxy
          visc_heat_xz(ix,iy,iz) = qxz(ix,iy,iz)*dvxz
          visc_heat_yz(ix,iy,iz) = qyz(ix,iy,iz)*dvyz
          visc_heat_xx(ix,iy,iz) = qxx(ix,iy,iz)*dvxdx
          visc_heat_yy(ix,iy,iz) = qyy(ix,iy,iz)*dvydy
          visc_heat_zz(ix,iy,iz) = qzz(ix,iy,iz)*dvzdz 

          visc_heat(ix,iy,iz) =   visc_heat_xy(ix,iy,iz) + visc_heat_xz(ix,iy,iz) + visc_heat_yz(ix,iy,iz) &
                                + visc_heat_xx(ix,iy,iz) + visc_heat_yy(ix,iy,iz) + visc_heat_zz(ix,iy,iz)
    ...
    DO iz = 0, nz+1
      ...
      DO iy = 0, ny+1
        ...
        DO ix = 0, nx+1
          ...
          dx_vx = (vx_xfc - vx_xfc_m)/dxb(ix)
          dx_vy = (vy_xfc - vy_xfc_m)/dxb(ix)
          dx_vz = (vz_xfc - vz_xfc_m)/dxb(ix)

          dy_vx = (vx_yfc - vx_yfc_m)/dyb(iy)         
          dy_vy = (vy_yfc - vy_yfc_m)/dyb(iy)
          dy_vz = (vz_yfc - vz_yfc_m)/dyb(iy)

          dz_vx = (vx_zfc - vx_zfc_m)/dzb(iz)
          dz_vy = (vy_zfc - vy_zfc_m)/dzb(iz)
          dz_vz = (vz_zfc - vz_zfc_m)/dzb(iz)         
         

          visc_heat_xx(ix,iy,iz) = qxx(ix,iy,iz)*(dx_vx)
          visc_heat_yy(ix,iy,iz) = qyy(ix,iy,iz)*(dy_vy)
          visc_heat_zz(ix,iy,iz) = qzz(ix,iy,iz)*(dz_vz) 
          visc_heat_xy(ix,iy,iz) = qxy(ix,iy,iz)*(dy_vx + dx_vy)
          visc_heat_xz(ix,iy,iz) = qxz(ix,iy,iz)*(dz_vx + dx_vz)
          visc_heat_yz(ix,iy,iz) = qyz(ix,iy,iz)*(dz_vy + dy_vz)
  ...
  SUBROUTINE eta_calc
    ...
    REAL(num) :: bxv, byv, bzv, fxv, fyv, fzv
    REAL(num) :: bmagv, jmagv, fmagv
    REAL(num) :: rhov, env, tpv
    

    IF (resistive_mhd) THEN
      eta_crit_cnt = 0.0_num
      DO iz = -1, nz+1
        izp = iz + 1
        DO iy = -1, ny+1
          iyp = iy + 1
          DO ix = -1, nx+1
            ...
            ! current at edge centre for neighbouring cell
            ...
            ! current at vertex
            jxv = (jxe + jxpe) * 0.5_num
            jyv = (jye + jype) * 0.5_num
            jzv = (jze + jzpe) * 0.5_num
            jmagv = SQRT(jxv**2 + jyv**2 + jzv**2)

            ! magnetic field components at vertex
            bxv = 0.25_num * (bx(ix,iy,izp) + bx(ix,iy,iz) + bx(ix,iyp,iz) + bx(ix,iyp,izp))
            byv = 0.25_num * (by(ix,iy,izp) + by(ix,iy,iz) + by(ixp,iy,iz) + by(ixp,iy,izp))
            bzv = 0.25_num * (bz(ix,iyp,iz) + bz(ix,iy,iz) + bz(ixp,iy,iz) + bz(ixp,iyp,iz))
            bmagv = SQRT(bxv**2 + byv**2 + bzv**2)                            
            
            ! rho at vertex            
            rhov = (rho(ix,iy,iz) + rho(ixp,iy,iz) + rho(ix,iyp,iz) + rho(ixp,iyp,iz) &
                    + rho(ix,iy,izp) + rho(ixp,iy,izp) + rho(ix,iyp,izp) + rho(ixp,iyp,izp))/8.0_num

            ! energy at vertex
            env = (energy(ix,iy,iz) + energy(ixp,iy,iz) + energy(ix,iyp,iz) + energy(ixp,iyp,iz) &
                   + energy(ix,iy,izp) + energy(ixp,iy,izp) + energy(ix,iyp,izp) + energy(ixp,iyp,izp))/8.0_num
                       
            ! temperature at vertex
            tpv = ((gamma - 1.0_num)*env)/2.0_num
                              
            !j_crit(ix, iy, iz) = j_crit_const                  
            ! the critical current for anomalous resistivity calculated using a local Larmor proton radius
            j_crit(ix, iy, iz) = (j_crit_const*((rhov*tpv)/bmagv))/MIN(dxc(ix),dyc(iy),dzc(iz))
            ! the critical current for anomalous resistivity calculated using a fixed Larmor proton radius
            !j_crit(ix, iy, iz) = j_crit_const*((rhov*SQRT(tpv))/MIN(dxc(ix),dyc(iy),dzc(iz)))
                       
            eta(ix, iy, iz) = eta_background 
            eta_num(ix, iy, iz) = 0.0_num
            IF (jmagv >= j_crit(ix, iy, iz)) THEN
              eta(ix, iy, iz) = eta(ix, iy, iz) + eta_crit
              eta_crit_cnt = eta_crit_cnt + 1.0_num              
              eta_num(ix, iy, iz) = (bmagv/SQRT(rhov))*MAX(dxc(ix),dyc(iy),dzc(iz))
            END IF 
  ...
  ! Use the subroutine rkstep
   SUBROUTINE resistive_effects
     ...
     REAL(num) :: half_dt, dt6, area, ohmic_ht_cc
     ...
     DO iz = 1, nz
       ...
       DO iy = 1, ny
         ...
         DO ix = 1, nx
           ...
           ! calculate the cell-centred Ohmic heating
           ohmic_ht_cc = (curlb(ix, iy, iz) + curlb(ixm, iy, iz) &
                          + curlb(ix, iym, iz) + curlb(ix, iy, izm) &
                          + curlb(ixm, iym, iz) + curlb(ixm, iy, izm) &
                          + curlb(ix, iym, izm) + curlb(ixm, iym, izm)) / 8.0_num

           energy(ix, iy, iz) = energy(ix, iy, iz) +  dt*ohmic_ht_cc / rho(ix, iy, iz)

           ohmic_heat(ix,iy,iz) = dt*cv(ix, iy, iz)*ohmic_ht_cc
           heating_ohmic = heating_ohmic + ohmic_heat(ix,iy,iz)
     ...
     DO iz = 0, nz
       DO iy = 0, ny
         DO ix = 0, nx
           ...
           heating_ohmic = heating_ohmic + w1
         END DO
       END DO
     END DO
